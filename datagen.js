	...
	try {
		for (p = 0; p < post.length; p++) {
			if (post[p]) {
				...
				if (post[p].title) {
					writeHtml("<h2><a href='#" + post[p].id + "' class='counter'>(" + post[p].id + ")</a> <a id='" + post[p].id + "' target='_blank' href='http://quadropolis.us/node/" + post[p].id + "'>" + post[p].title + "</a></h2>");
				} else {
					writeHtml("<h2><a href='#" + post[p].id + "' class='counter'>(" + post[p].id + ")</a> Untitled Post</h2>")
				};
				...
				if (post[p].image) {
					if (post[p].image.includes("quadropolis.us/files/")) {
						imagename = post[p].image.substr(22).replace(/%20/g, "-").toLowerCase();
					} else {
						imagename = post[p].image;
					};
					writeHtml ('<img alt=" this screenshot was not originally available or there was a problem extracting it" src="' + imagename + '" loading="lazy"/>');
				};
				...
				if (post[p].hrefs.length > 0) {
					for (u = 0; u < post[p].hrefs.length; u++) {
						if (post[p].hrefs[u].includes("quadropolis.us/files/")) {
							filename = post[p].hrefs[u].substr(22).replace(/%20/g, "-").toLowerCase();
